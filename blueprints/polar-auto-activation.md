...
- Real inputs, in full:
  ...
  - Plans and prices (owner will create these as three products in Polar): Starter $47/month, Pro $97/month, Agency $1,497/month. (Setup fees, if any, are configured Polar-side as one-time products; the webhook treats any paid order for a mapped product as activation.)
...
- Data shapes / examples:
  - Sample `subscription.active` payload core: `{"type":"subscription.active","data":{"id":"sub_x","product_id":"prod_pro_id","customer":{"email":"customer@example.com"},"status":"active"}}` → expected effect: upsert `subscriptions` row for the auth user with that email: `plan='pro', status='active'`.
...
- Gotchas:
  ...
  - `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are auto-injected env vars inside deployed Edge Functions. `POLAR_WEBHOOK_SECRET` and the three product-ID vars must be set by the owner via `supabase secrets set`.
...
1. Create `docs/POLAR_SETUP.md` containing, in order: (a) SQL block for the owner to run once in the Supabase SQL editor:
   ...
   ```
   (b) Polar dashboard click-path: create three subscription products — "MagnetEngine Starter" $47/mo, "MagnetEngine Pro" $97/mo, "MagnetEngine Agency" $1,497/mo — copy each product ID and each checkout link; (c) set the three checkout links in `.env` as `VITE_PAYMENT_LINK_STARTER|PRO|AGENCY` and redeploy the frontend; (d) `supabase functions deploy polar-webhook --no-verify-jwt`; (e) in Polar → Settings → Webhooks, add endpoint `https://<project-ref>.supabase.co/functions/v1/polar-webhook` with events `order.paid`, `subscription.active`, `subscription.canceled`, `subscription.revoked`, copy the signing secret; (f) `supabase secrets set POLAR_WEBHOOK_SECRET=<secret> POLAR_PRODUCT_ID_STARTER=<id> POLAR_PRODUCT_ID_PRO=<id> POLAR_PRODUCT_ID_AGENCY=<id>`; (g) the curl smoke test from step 4.
2. Create `supabase/functions/polar-webhook/index.ts`:
   ...
   - Parse event. Build `PRODUCT_TO_PLAN: Record<string,string>` from the three `POLAR_PRODUCT_ID_*` env vars → 'starter'|'pro'|'agency'.
   ...
   - For `order.paid` and `subscription.active`: plan = `PRODUCT_TO_PLAN[productId]`; if productId unmapped, default plan to 'starter' and `console.warn`. Upsert `subscriptions` `{ user_id, plan, status:'active', updated_at: new Date().toISOString() }` with `onConflict:'user_id'`.
...
3. Edit `src/pages/PendingActivationPage.tsx` — replace the body of `paymentHref` so the buyer's checkout email is prefilled (this makes email-matching reliable):
   ```typescript
   const paymentHref = (tier: PlanTier) => {
       const base = PAYMENT_LINKS[tier];
       if (!base) return UPGRADE_CONTACT;
       if (!user?.email) return base;
       const sep = base.includes('?') ? '&' : '?';
       return `${base}${sep}customer_email=${encodeURIComponent(user.email)}`;
   };
   ```
   Directly above the "Already paid?" card's existing text, change the sentence `Already paid? Once we confirm your payment your account unlocks automatically.` to `Already paid? Your account unlocks automatically within a minute of payment — click below to refresh.` (exact copy).
...
5. Update `.env.example`: change the comment above `VITE_PAYMENT_LINK_*` lines to `# Polar.sh checkout links (see docs/POLAR_SETUP.md)`.
...
- Copy / values / snippets to use verbatim: the SQL function in step 1, the `paymentHref` snippet and the replacement sentence in step 3, env var names `POLAR_WEBHOOK_SECRET`, `POLAR_PRODUCT_ID_STARTER`, `POLAR_PRODUCT_ID_PRO`, `POLAR_PRODUCT_ID_AGENCY`.
...
[ ] Edge case handled: unknown event type, missing email, unmatched email, and unmapped product ID all return 202 without writing, with a console.warn.
[ ] `paymentHref` appends `customer_email` only when a payment link is configured and a user email exists.
[ ] `docs/POLAR_SETUP.md` contains the SQL, the six owner steps, all four env secret names, and the curl 403 smoke test — no placeholder text like TBD remains.
