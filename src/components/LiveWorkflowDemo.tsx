...
const ConnectVisual: React.FC = () => (
  <div className="relative w-full h-full flex items-center justify-center">
    ...
    >
      <line x1="16" y1="22" x2="50" y2="50" stroke="rgba(249,115,22,0.4)" strokeWidth="0.7" strokeDasharray="4,3">
        <animate attributeName="stroke-dashoffset" from="7" to="0" dur="1.3s" repeatCount="indefinite" />
      </line>
      <line x1="84" y1="22" x2="50" y2="50" stroke="rgba(249,115,22,0.4)" strokeWidth="0.7" strokeDasharray="4,3">
        <animate attributeName="stroke-dashoffset" from="7" to="0" dur="1.6s" repeatCount="indefinite" />
      </line>
      <line x1="16" y1="78" x2="50" y2="50" stroke="rgba(251,191,36,0.4)" strokeWidth="0.7" strokeDasharray="4,3">
        <animate attributeName="stroke-dashoffset" from="7" to="0" dur="1.9s" repeatCount="indefinite" />
      </line>
      <line x1="84" y1="78" x2="50" y2="50" stroke="rgba(251,191,36,0.4)" strokeWidth="0.7" strokeDasharray="4,3">
        <animate attributeName="stroke-dashoffset" from="7" to="0" dur="1.5s" repeatCount="indefinite" />
      </line>
      {/* Travelling dots */}
      <circle r="1.2" fill="rgba(249,115,22,0.9)">
      ...
      <circle r="1.2" fill="rgba(251,191,36,0.9)">
    ...
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
      <div className="w-20 h-20 rounded-2xl bg-brand-500/15 border border-brand-500/40 flex flex-col items-center justify-center shadow-[0_0_30px_rgba(249,115,22,0.25)]">
        <Magnet size={24} className="text-brand-400" />
        <span className="text-[8px] text-brand-400 mt-1 font-semibold tracking-tight text-center leading-tight">
          Magnet<br />Engine
        </span>
      </div>
    </div>
    ...
    <div className="absolute" style={{ top: '10%', left: '5%' }}>
      <div className="w-16 h-16 rounded-xl bg-surface border border-white/10 flex flex-col items-center justify-center gap-0.5">
        <span className="text-base font-black text-[#0A66C2] leading-none">in</span>
        <span className="text-[8px] text-neutral-500">LinkedIn</span>
      </div>
    </div>
    ...
    <div className="absolute" style={{ top: '10%', right: '5%' }}>
      <div className="w-16 h-16 rounded-xl bg-surface border border-white/10 flex flex-col items-center justify-center gap-0.5">
        ...
        <span className="text-[8px] text-neutral-500">CSV List</span>
      </div>
    </div>
    ...
    <div className="absolute" style={{ bottom: '10%', left: '5%' }}>
      <div className="w-16 h-16 rounded-xl bg-surface border border-white/10 flex flex-col items-center justify-center gap-0.5">
        <Database size={17} className="text-neutral-400" />
        <span className="text-[8px] text-neutral-500">CRM</span>
      </div>
    </div>
    ...
    <div className="absolute" style={{ bottom: '10%', right: '5%' }}>
      <div className="w-16 h-16 rounded-xl bg-surface border border-white/10 flex flex-col items-center justify-center gap-0.5">
        <span className="text-sm font-bold text-neutral-400 font-mono">{'{}'}</span>
        <span className="text-[8px] text-neutral-500">Webhook</span>
      </div>
    </div>
    ...
    <div className="w-full h-full flex flex-col gap-2 px-4 py-3">
          ...
          >
            <div
              className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                lead.pass ? 'bg-brand-500/20 text-brand-400' : 'bg-red-500/20 text-red-400'
              }`}
            >
              {lead.name[0]}
            </div>
            <span className="text-xs text-neutral-300 flex-1 truncate">{lead.name}</span>
            <span className={`text-xs font-mono shrink-0 ${lead.pass ? 'text-brand-400' : 'text-red-400'}`}>
              {lead.score}%
            </span>
            {lead.pass ? (
      ...
      <div className="flex items-center gap-8 pt-2 mt-1 border-t border-white/5 w-full justify-center">
        <div className="text-center">
          <div className="text-2xl font-medium text-brand-400">312</div>
          <div className="text-[9px] text-neutral-500 uppercase tracking-widest">Qualified</div>
        </div>
        <div className="w-px h-8 bg-white/10" />
        <div className="text-center">
          <div className="text-2xl font-medium text-red-400/70">2,535</div>
          <div className="text-[9px] text-neutral-500 uppercase tracking-widest">Filtered Out</div>
        </div>
      </div>
    </div>
  );
    ...
    <div className="w-full h-full flex flex-col gap-3 p-5">
      ...
      <div className="flex items-center gap-3 pb-3 border-b border-white/5">
        ...
        <div className="min-w-0">
          ...
          <div className="text-[11px] text-neutral-500 truncate">CEO @ ScaleMax Agency · 3rd+</div>
        </div>
        <div className="ml-auto shrink-0 flex items-center gap-1.5 text-[9px] text-info-400 bg-info-500/10 px-2.5 py-1 rounded-full border border-info-500/20">
          <div className="w-1 h-1 rounded-full bg-info-400 animate-pulse" />
          AI Analyzing
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-2 min-h-0">
        <div className="text-[10px] text-neutral-500 uppercase tracking-widest">Generated Message</div>
        <div className="flex-1 bg-surface border border-white/5 rounded-xl p-4 text-[13px] text-neutral-300 leading-relaxed overflow-hidden">
          {FULL_MESSAGE.slice(0, charIndex)}
          <span className="inline-block w-0.5 h-[1em] bg-brand-400 ml-px animate-pulse align-middle" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          ['High', 'Relevance', 'text-brand-400'],
          ['2.3s', 'Generated', 'text-info-400'],
        ].map(([val, label, color]) => (
          <div key={label} className="text-center bg-white/[0.02] border border-white/5 rounded-lg py-2">
            ...
            <div className="text-[9px] text-neutral-500 uppercase tracking-widest">{label}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="w-full h-full flex flex-col gap-2 p-4">
      <div className="flex items-center justify-between">
        <span className="text-xs text-neutral-400 font-medium">This Week's Calendar</span>
        <div className="flex items-center gap-1.5 text-[9px] text-brand-400 bg-brand-500/10 px-2.5 py-1 rounded-full border border-brand-500/20">
          <Rocket size={9} />
      ...
      <div className="flex-1 space-y-1">
        {slots.map((slot) => (
          ...
          >
            <span className="text-[11px] text-neutral-500 w-[72px] shrink-0 font-mono">{slot.time}</span>
            {slot.booked ? (
              <>
                <div className="w-5 h-5 rounded-full bg-brand-500/20 flex items-center justify-center shrink-0">
                  <span className="text-[9px] font-bold text-brand-400">{slot.name[0]}</span>
                </div>
                <span className="text-xs text-brand-300 flex-1">{slot.name}</span>
                <span className="text-[9px] font-semibold uppercase tracking-wider text-brand-500 bg-brand-500/10 px-2 py-0.5 rounded-full shrink-0">
                  Booked
                </span>
              </>
            ) : (
      ...
      <div className="flex items-center justify-between pt-2 border-t border-white/5">
        <span className="text-[11px] text-neutral-500">Running 24/7 on autopilot</span>
        <span className="text-sm font-medium text-brand-400">
          +14 calls{' '}
          <span className="text-xs text-neutral-500 font-normal">this week</span>
        </span>
      </div>
    </div>
  );
...
const LiveWorkflowDemo: React.FC = () => {
  ...
  return (
    <section className="py-32 px-6 relative bg-surface" id="workflow">
      <div className="max-w-6xl mx-auto">
        {/* Section Header */}
        <div className="text-center mb-20 space-y-4">
          <div className="text-[10px] font-semibold uppercase tracking-widest text-neutral-500 mb-2">
            How It Works
          </div>
        ...
        <div className="grid lg:grid-cols-12 gap-16 items-center max-w-5xl mx-auto">
          ...
          <div className="lg:col-span-5 space-y-1">
            {steps.map((step, index) => {
              ...
              return (
                ...
                >
                  {isActive ? (
                    <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-brand-500 shadow-[0_0_10px_rgba(249,115,22,0.8)] rounded-full" />
                  ) : (
                    <div className="absolute left-[0.5px] top-0 bottom-0 w-[1px] bg-white/10" />
                  )}
                  <div className="flex items-start gap-4">
                    <div className={`mt-1 shrink-0 ${isActive ? 'text-brand-400' : 'text-neutral-500'}`}>
                      <Icon size={20} strokeWidth={1.5} />
                    </div>
                    <div>
                      <h4 className="text-white font-medium mb-2 flex items-center gap-2">
                        <span className="text-neutral-500 text-sm font-mono mr-1">{step.id} —</span>
                        {step.title}
                      </h4>
          ...
          <div className="lg:col-span-7 relative w-full h-[400px]">
            {/* Background glow */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-[120%] bg-brand-500/10 blur-[100px] rounded-full pointer-events-none -z-10" />

            {/* Panel window */}
            <div className="relative w-full h-full bg-surface-raised border border-white/5 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
              {/* Window chrome */}
              <div className="h-10 border-b border-white/5 flex items-center px-4 gap-2 bg-surface shrink-0">
