...
//
// Auth model (two layers), both handled by servePost:
//   1. The gateway requires a valid Supabase JWT (function deployed WITHOUT
//      --no-verify-jwt).
//   2. servePost({ adminOnly: true }) resolves the JWT to a user and requires
...
servePost<Body>("admin-api", async ({ body, sb }) => {
