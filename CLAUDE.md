...
SOP_DOC_URL / CRM_SHEET_URL / ONBOARDING_CALL_URL  # onboarding email links (optional; call URL defaults to cal.com/magnetengine/30min)
```
...
4. **Onboarding / setup guide** (onboarding-call booking link + SOP doc + CRM tracker sheet; source content in `docs/sops/`) — scheduled +15 min after payment by `whop-webhook`, or sent immediately by `admin-api` on manual activation.
